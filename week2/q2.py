...
class History:
    ...
    def is_win(self):
        # In Notakto the game has ended (in the misere sense) once every board is
        # inactive (dead, i.e. contains a three in a row) -- there are no more legal
        # moves left for anyone.
        return sum(self.active_board_stats) == 0

    def get_valid_actions(self):
        # Valid actions are empty squares ('0') on boards that are still active.
        # Move ordering is mandatory: within a board, search center, then corners,
        # then edges -- i.e. position order [4, 0, 2, 6, 8, 1, 3, 5, 7]. We loop the
        # position order on the outside and the boards on the inside so that, e.g.,
        # every board's center square is tried before any board's corner square.
        move_order = [4, 0, 2, 6, 8, 1, 3, 5, 7]
        valid_actions = []
        for pos in move_order:
            for board_num in range(self.num_boards):
                if self.active_board_stats[board_num] == 1 and self.boards[board_num][pos] == '0':
                    valid_actions.append(board_num * 9 + pos)
        return valid_actions

    def is_terminal_history(self):
        # The game ends exactly when there are no active boards left (no legal moves).
        return self.is_win()

    def get_value_given_terminal_history(self):
        # Misere convention: whichever player made the move that resulted in the
        # last board becoming inactive (i.e. the player who moved last) loses the
        # game. Player 1 moved last iff len(self.history) is odd (player 1 moves on
        # move numbers 0, 2, 4, ... i.e. even indices, so after an odd number of total
        # moves the last move was made by player 1).
        # Value is from player 1's (maximizer's) perspective: +1 if player 1 wins,
        # -1 if player 1 loses.
        if len(self.history) % 2 == 1:
            # player 1 made the last move -> player 1 loses
            return -1
        else:
            # player 2 made the last move -> player 2 loses, player 1 wins
            return 1

    def get_next_history_obj(self, action):
        # Helper to build the next History object after playing `action`.
...
def alpha_beta_pruning(history_obj, alpha, beta, max_player_flag):
    ...
    if max_player_flag:
        ...
        for action in valid_actions:
            child = history_obj.get_next_history_obj(action)
...
def maxmin(history_obj, max_player_flag):
    ...
    boards_key = history_obj.get_boards_str()
    if boards_key in board_positions_val_dict:
        return board_positions_val_dict[boards_key]

    if history_obj.is_terminal_history():
        value = history_obj.get_value_given_terminal_history()
        board_positions_val_dict[boards_key] = value
        return value
